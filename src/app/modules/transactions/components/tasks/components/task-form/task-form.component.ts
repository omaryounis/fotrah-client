...
export class TaskFormComponent implements OnInit {
 ...
 statusTypes  = StatusEnum;
  ...
  constructor(private dynamicDialogConfig: DynamicDialogConfig, private taskService: TasksService ,private reasonService :VoteReasonsService) { }
  ...
  ngOnInit(): void {
    ...
    this.updateData();
    this.taskService.getTaskDetails(parseInt(this.taskData.id!)).subscribe(
      res => {
  ...
  downloadFile(base64File: string, fileName: string , isBase64 :boolean) {
    ...
    if (isBase64) {
      ...
      const fileUrl = base64File.replace(environment.filePath, window.origin  + '/objections/'); //test// Convert backslashes to forward slashes for file protocol
 ...
 return formattedArray;
  ...
  getApproveLabel() :string {
     ...
     if (this.taskData.requestStatusId == StatusEnum.PENDING && this.taskData.requestTypeId == RequestTypes.OBJECTION) {
        this.is_valid_reject = false;
        return 'proceed';
     }
     if (this.taskData.requestStatusId == StatusEnum.APPROVED && this.taskData.requestTypeId == RequestTypes.OBJECTION) {
        this.is_valid_reject = true;
        this.is_voting = true
        return 'vote-accept';
     }
     if ((this.taskData.requestStatusId == StatusEnum.COMPLETED || this.taskData.requestStatusId == StatusEnum.REJECTED) && this.taskData.requestTypeId == RequestTypes.OBJECTION) {
        this.is_valid_reject = false;
